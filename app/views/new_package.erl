<div class="row-fluid">
	...
	<div class="span10">
		<div class="alert alert-info" ng-show='selectedDataset && selectedPackage'>
			Create a <strong>{{selectedPackage.name}}</strong>
			machine with <strong>{{selectedDataset.name}}</strong>
			called: <input class='input-large' style='margin-left: 8px;' type=text ng-model='name' autofocus>
		</div>
	</div>

	<button class='btn btn-success pull-right' ng-show='name' ng-click='create_machine()'>Create!</button>
	...
	<div class="span5">
		<div class="thumbnail">
			...
			<table class="table table-hover small">
				...
				<tr class='clickable'  ng-repeat='d in datasets' ng-click='click_dataset(d)' ng-class="selectedDataset.dataset==d.dataset && 'success'">
